    ...
    <div className="min-h-screen bg-white text-black">
      <div className="max-w-3xl mx-auto p-8 print:p-0">
        <div className="flex justify-end mb-4 print:hidden">
          <Button onClick={() => window.print()} data-testid="button-print-order">
            <Printer className="h-4 w-4 mr-2" />
            Печать
          </Button>
        </div>

        <h1 className="text-xl font-bold text-center mb-1">Заказ покупателя</h1>
        <p className="text-center text-sm mb-6">
          №{order.id.slice(0, 8)} от {dateFormatter.format(new Date(order.createdAt))}
        </p>
        ...
        <div className="grid grid-cols-2 gap-4 text-sm mb-6">
          ...
          <div>
            <div className="font-semibold">Статус оплаты:</div>
            <div>{order.isPaid ? "Оплачен" : "Не оплачен"}</div>
          </div>
        </div>
        ...
        <table className="w-full text-sm border-collapse mb-6">
          <thead>
            <tr className="border-b-2 border-black">
              ...
              <th className="text-right py-1.5 pr-2">Отгружено</th>
          ...
          <tbody>
            {order.items.map((item, index) => (
              <tr key={item.id} className="border-b border-gray-300">
                ...
                <td className="text-right py-1.5 pr-2">{item.shippedQuantity}</td>
          ...
          <tfoot>
            <tr>
              ...
              <td className="text-right font-semibold py-2">{currency.format(order.totalAmount)}</td>
            </tr>
          </tfoot>
        </table>
        ...
        <div className="grid grid-cols-2 gap-8 text-sm mt-16">
          <div>
            <div className="mb-8">Заказ принял: _____________________ / _____________________ /</div>
            <div className="text-xs">(должность, подпись)&emsp;&emsp;&emsp;&emsp;&emsp;(расшифровка подписи)</div>
          </div>
          <div>
            <div className="mb-8">Заказ согласовал: _____________________ / _____________________ /</div>
            <div className="text-xs">(должность, подпись)&emsp;&emsp;&emsp;&emsp;&emsp;(расшифровка подписи)</div>
          </div>
        </div>
