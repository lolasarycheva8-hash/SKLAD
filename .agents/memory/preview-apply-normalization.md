...
Zero-width formatting characters that are visually absent must be removed, not replaced with spaces, before whitespace normalization. Replacing them with spaces fails to equate visually identical values such as `AB` and `A<zero-width>B`.